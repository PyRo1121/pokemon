...
class TwitchBot(tcommands.Bot):
    ...
    async def event_ready(self):
        print(
            f"Connected to Twitch as {TWITCH_CHANNEL} | Monitoring: {TWITCH_MONITOR_CHANNEL}"
        )
        ...
        try:
            ...
            if author_name == "pokemoncommunitygame" and "wild" in content.lower():
                try:
                    ...
                    print(
                        f"Pokemon spawn detected: {pokemon_name} {'(SHINY!)' if is_shiny else ''}"
                    )
    ...
    async def get_pokemon_image(self, pokemon_name):
        try:
            ...
            animated_url = (
                f"https://play.pokemonshowdown.com/sprites/ani/{clean_name}.gif"
            )
            ...
            shiny_animated_url = (
                f"https://play.pokemonshowdown.com/sprites/ani-shiny/{clean_name}.gif"
            )
            ...
            response = self.session.get(
                f"https://pokeapi.co/api/v2/pokemon/{clean_name}"
            )
            if response.status_code == 200:
                data = response.json()
                sprite_options = [
    ...
    async def combine_pokemon_info(self):
        if self.last_pcg_message and self.last_pib_message:
            try:
                ...
                pokemon_info = dict(
                    item.split(": ") for item in self.last_pcg_message.split("|")
                )
                ...
                # Create embed
                embed = discord.Embed(title=f"✨ {pokemon_name} ✨", color=embed_color)
                ...
                type_text = " ".join(
                    f"{self.TYPE_EMOJIS.get(t, '')} {t.title()}" for t in types
                )
                ...
                if stats.get("balls"):  # Use .get() to avoid KeyError
                    best_ball = max(
                        stats["balls"], key=self.calculate_catch_probability
                    )
                    ...
                    embed.add_field(
                        name="📦 Catch Rates", value=catch_text, inline=False
                    )

                # Add evolution info if available
                if self.last_evolution_info:
                    evo_info = self.last_evolution_info.replace(
                        "Evolution:", ""
                    ).strip()
                    embed.add_field(
                        name="⚡ Evolution Chain", value=evo_info, inline=False
                    )

                # Add extra info if available
                if self.last_extra_info:
                    if "Database Stats:" in self.last_extra_info:
                        stats = self.last_extra_info.replace(
                            "Database Stats:", ""
                        ).strip()
                        embed.add_field(
                            name="📈 Spawn Statistics", value=stats, inline=False
                        )
                    elif "Ball Effectiveness:" in self.last_extra_info:
                        balls = self.last_extra_info.replace(
                            "Ball Effectiveness:", ""
                        ).strip()
                        embed.add_field(
                            name="🎯 Enhanced Catch Rates", value=balls, inline=False
                        )
                ...
                if hp_speed:
                    embed.add_field(
                        name="💪 Key Stats", value="\n".join(hp_speed), inline=True
                    )
...
class DiscordBot(discord.Client):
    def __init__(self, *args, **kwargs):
        ...
        @self.tree.command(
            name="sync", description="Force sync slash commands (Admin only)"
        )
        async def sync(interaction: discord.Interaction):
            if interaction.user.guild_permissions.administrator:
                ...
                await interaction.response.send_message(
                    "You need admin permissions to sync commands!", ephemeral=True
                )

        @self.tree.command(
            name="setup", description="Set the channel for Pokemon spawn notifications"
        )
        async def setup(
            ...
            try:
                ...
                await interaction.response.send_message(
                    f"❌ Error setting up channel: {str(e)}", ephemeral=True
                )
    ...
    async def lastspawn(self, interaction: discord.Interaction):
        ...
        if spawn:
            ...
            embed.add_field(
                name="Details", value=f"Tier: {spawn['tier']}\nTypes: {spawn['types']}"
            )
    ...
    async def post_pokemon_info(self):
        ...
        print(
            f"Connected to Discord channel: {channel.name} (ID: {DISCORD_CHANNEL_ID})"
        )
        print(
            f"Bot permissions in channel: {channel.permissions_for(channel.guild.me)}"
        )
