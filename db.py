...
class PokemonDB:
    ...
    async def add_spawn(self, pokemon_data: Dict):
        with sqlite3.connect(self.db_path) as conn:
            ...
            ''', (pokemon_data['name'], 1 if pokemon_data['is_shiny'] else 0,
    ...
    async def get_spawn_stats(self) -> Dict:
        with sqlite3.connect(self.db_path) as conn:
            ...
            c.execute('''
                ...
                FROM spawns
            ''')
            return dict(zip(['total_spawns', 'total_shinies', 'unique_pokemon'], c.fetchone()))
    ...
    async def get_rarest_spawns(self, limit: int = 5) -> List[Dict]:
        with sqlite3.connect(self.db_path) as conn:
            ...
            c.execute('''
                ...
                LIMIT ?
            ''', (limit,))
            return [dict(zip(['name', 'count', 'shinies', 'last_seen'], row))
