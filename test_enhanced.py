...
from config import *
from pokemon import DiscordBot, TwitchBot
...
class TestDiscordBot(DiscordBot):
    async def run_test_sequence(self):
        ...
        await send_test_message(
            self, "pokemoncommunitygame", "ACTION ⭐⭐⭐ A wild Mewtwo appears! ⭐⭐⭐"
        )
        await send_test_message(
            self,
            "pokeinfobot",
        ...
        print("\nTest Case 2: Special Ball Effectiveness")
        await send_test_message(
            self, "pokemoncommunitygame", "ACTION A wild Gyarados appears!"
        )
        await send_test_message(
            self,
            "pokeinfobot",
        ...
        print("\nTest Case 3: Evolution and Status Info")
        await send_test_message(
            self, "pokemoncommunitygame", "ACTION A wild Charmeleon appears!"
        )
        await send_test_message(
            self,
            "pokeinfobot",
